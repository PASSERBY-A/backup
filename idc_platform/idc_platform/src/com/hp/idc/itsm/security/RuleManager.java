...
import com.hp.idc.itsm.security.rule.AllowAllRuleInfo;
import com.hp.idc.itsm.security.rule.AllowLocalgroupInfo;
import com.hp.idc.itsm.security.rule.AllowOrganizationInfo;
import com.hp.idc.itsm.security.rule.AllowPersonInfo;
import com.hp.idc.itsm.security.rule.AllowWorkgroupInfo;
import com.hp.idc.itsm.security.rule.RejectAllRuleInfo;
import com.hp.idc.itsm.security.rule.RejectLocalgroupInfo;
import com.hp.idc.itsm.security.rule.RejectOrganizationInfo;
import com.hp.idc.itsm.security.rule.RejectPersonInfo;
import com.hp.idc.itsm.security.rule.RejectWorkgroupInfo;
import com.hp.idc.itsm.security.rule.RuleInfo;
...
public class RuleManager {
	 ...
	 * 存储规则缓存
	 */
	public static Map ruleCache = new HashMap();
	...
	public static boolean valid(String user, String rule, boolean def) {
		PersonInfoInterface pInfo = PersonManager.getPersonById(user);
		if (pInfo == null)
			return def;
		if (user.equalsIgnoreCase("root")){
			return true;
		}
		List ruleinfos = analyseRule(rule);
		if (ruleinfos.size() == 0)
			return def;
	
		OrganizationInfoInterface orgInfo = pInfo.getOrganization();
		String orgId = "";
		if (orgInfo != null)
			orgId = orgInfo.getId();
		List wgs = pInfo.getWorkgroups();
		String ids[] = new String[wgs.size()];
		for (int i = 0; i < ids.length; i++) {
			ids[i] = ((CIInfo)wgs.get(i)).getId();
		}
	 ...
	 * @return List<IRuleInfo>
	 */
	public static List analyseRule(String rule) {
		if (rule == null || rule.equals("") || rule.trim().equals(""))
			return new ArrayList();
		List ruleList = (List)ruleCache.get(rule);
		if (ruleList != null)
			return ruleList;
		...
		for (int i = 0; i < rules.length; i++) {
			...
			} else if (ruleName.equalsIgnoreCase("al")) {//允许本地工作组
				rInfo = new AllowLocalgroupInfo();
			} else if (ruleName.equalsIgnoreCase("rl")) {//拒绝本地工作组
				rInfo = new RejectLocalgroupInfo();
			} else {
				continue;
			}
	 ...
	 * @return
	 */
	public static Map parseRuleStrByCategoryToMap(String ruleStr) {
	 ...
	 * @return
	 */
	public static List parseRuleStrByCategoryToList(String ruleStr) {
		if (ruleStr==null || ruleStr.length()==0) {
			return null;
		}
		String[] rules = ruleStr.split(";");
	...
	public static String ruleDesc(String rule) {
		String retStr = "";
		List ruleList = analyseRule(rule);
		if (ruleList.size()==0)
			return "没做限制";
