...
Model model = ServiceManager.getModelService().getModelById("person");
...
var store = new Ext.data.GroupingStore({
	proxy: new Ext.data.HttpProxy({
		url: '/resm/interface/resQuery.jsp'
	}),
	baseParams: { "modelId": "person", limit: <%=pageCount%> },
...
var sf = new Ext.form.SearchField({
		paramName:'filter',
		store: store,
		emptyText : 'ID相同，或姓名模糊匹配，或者手机按结尾匹配（至少4位），或按邮箱@前方匹配，或者部门相同',
		width: 560
	});
...
var store2 = new Ext.data.GroupingStore({
	proxy: new Ext.data.HttpProxy({
		url: '/resm/interface/resQuery2.jsp'
	}),
	...
	height: 200,
		...
		handler : function() {
				...
				for (i = 0; i < count; i++)
				{
					...
					if (i > 0)
						ret += ",";
<% if (request.getParameter("idOnly") != null) { %>
						ret += r.get("id");
<% } else { %>
...
mainPanel.on("rowdblclick", function(g, rowIndex, e) {
	var cell = g.getStore().getAt(rowIndex);
<% if (request.getParameter("idOnly") != null) { %>
	top.window.returnValue = cell.get("id");
<% } else { %>
	top.window.returnValue = cell.get("id") + "=" + cell.get("name");
<% } %>
	top.window.close();
