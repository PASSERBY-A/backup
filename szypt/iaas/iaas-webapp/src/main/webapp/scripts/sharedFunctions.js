...
cloudStack.converters = {
    ...
    toCloudDate:function(date,format){
    	 var localDate = "";
    	 if(date!=null){
    		 var year=1900+parseInt(date.year);
    		 var month=parseInt(date.month)+1;
    		 var day=date.date;
    		 localDate=year+"-"+month+"-"+day;
    		 if(format=="1"){
    			 localDate += " "+date.hours + ":" + date.minutes + ":" + date.seconds ;
    		 }
    	 }
    	 return localDate;
    },
    toToday:function(date,format){
    ...
    convertByType2: function(alertCode, total, used) {
        switch (alertCode) {
	        case 1:
	        	var unused = total - used;
	        	total = cloudStack.converters.convertHz(total);
	        	used = cloudStack.converters.convertHz(used);
	        	if (total != "" && used != "" && total.split(" ")[1] == used.split(" ")[1]) {
	        		var temp1 = total.split(" ")[0];
	        		var temp2 = used.split(" ")[0];
	        		
	        		return (temp1 - temp2).toFixed(2) + total.split(" ")[1];
	        	} else {
	        		return cloudStack.converters.convertByType(alertCode, unused);
	        	}
            case 0:
            case 2:
            case 3:
            ...
            case 11:
            	var unused = total - used;
            	total = cloudStack.converters.convertBytes(total);
	        	used = cloudStack.converters.convertBytes(used);
	        	if (total != "" && used != "" && total.split(" ")[1] == used.split(" ")[1]) {
	        		var temp1 = total.split(" ")[0];
	        		var temp2 = used.split(" ")[0];
    ...
    convertTemplateAccountTypeStr:function(accountType){
