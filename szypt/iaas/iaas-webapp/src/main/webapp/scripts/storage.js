...
(function(cloudStack) {
    ...
    var volumeActionfilter = function(args) {
        ...
        var boolean=false;
        $.ajax({
            url: createURL("getRelResourcePoolByZoneId&cmsz=yes&response=json&zoneId="+jsonObj.zoneid),
            dataType: "json",
            async: false,
            success: function(json) {
                if(json.root=="R_01"){
    ...
    var snapshotActionfilter = function(args) {
        ...
        var allowedActions = [];
        var volumeid=jsonObj.volumeid;
        var zoneid=null;
        $.ajax({
            url: createURL("listVolumes&response=json&id="+volumeid),
            dataType: "json",
            async: false,
            success: function(json) {
            	zoneid=json.listvolumesresponse.volume[0].zoneid;
            }
        });
        var boolean=false;
        $.ajax({
            url: createURL("getRelResourcePoolByZoneId&cmsz=yes&response=json&zoneId="+zoneid),
            dataType: "json",
            async: false,
            success: function(json) {
            	if(json.root=="R_01"){
